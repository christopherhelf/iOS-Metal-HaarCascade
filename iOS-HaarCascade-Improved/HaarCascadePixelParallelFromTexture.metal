//
//  HaarCascadePixelParallelFromTexture.metal
...
kernel void haarCascadePixelParallelFromTexture(
                                                ...
                                                texture2d<float, access::read> sqIntegralImage [[texture(3)]],
                                                ...
                                                device atomic_int &outMaskCounter [[ buffer(4) ]],
                                                
                                                device int* outMask [[ buffer(5) ]],
                                                constant int &step [[ buffer(6) ]],
                                                
                                                texture2d<float, access::write> varianceMap [[texture(4)]],
                                                
                                                uint2 blockIdx [[threadgroup_position_in_grid]],
                                                uint2 threadIdx [[ thread_position_in_threadgroup ]]

) {
    // base variables, to be filled depending whether this is an active thread
    uint yOffset;
    uint xOffset;
    uint outMaskVal;
    bool isInactiveThread = false;
    
    // variance, to be set in first pass
    float normVariance = 0.0;
    
    // current position
    yOffset = blockIdx.y * step;
    xOffset = (blockIdx.x * numThreadsAnchorsParallel + threadIdx.x) * step;
    ...
    // invalid values
    if (
        xOffset + window.width >= integralImage.get_width() ||
        yOffset + window.height >= integralImage.get_height())
    {
        isInactiveThread = true;
    }
    
    // calculate variance only for active threads
    if (!isInactiveThread) {
        
        // set the mask
        outMaskVal = (yOffset << 16) | xOffset;
        
        // calculate the norm variance
        float sqSumPix = calculateSum(sqIntegralImage, window.x, window.y, window.width, window.height);
        float sumPix = calculateSum(integralImage, window.x, window.y, window.width, window.height);
        normVariance = sqSumPix/haarCascade.scaledArea - pow(sumPix/haarCascade.scaledArea,2);
        
        // we take the square root here
        if (normVariance >= 0.f) {
            normVariance = sqrt(normVariance);
        } else {
            normVariance = 1.0f;
        }
    ...
    // write out the variance map
    if (bPass && !isInactiveThread) {
        varianceMap.write(float4(normVariance),uint2(xOffset,yOffset));
    }
